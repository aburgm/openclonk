...
void C4FoWDrawLightTextureStrategy::End(const StdProjectionMatrix& projectionMatrix, const C4Shader& renderShader, const C4Shader& directionalRenderShader)
{
	...
	const float light_dir[2] = { 1.0, 0.0 }; // TODO: take from pLight
	directionalRenderCall.SetUniform2fv(C4FoWRSU_LightDirection, 1, light_dir);
	directionalRenderCall.SetUniform1f(C4FoWRSU_LightAngularRangeCos, cos(20.0 * M_PI / 180.0));
	directionalRenderCall.SetUniform1f(C4FoWRSU_LightAngularFadeCos, cos(50.0 * M_PI / 180.0));
	directionalRenderCall.SetUniform1f(C4FoWRSU_LightOmniDistance, 5.0);
	directionalRenderCall.SetUniform1f(C4FoWRSU_LightOmniDistanceFade, 20.0);
